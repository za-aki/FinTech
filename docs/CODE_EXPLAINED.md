...
*   Deduplication: We found exactly 400 identical duplicate rows in the raw transaction file. We dropped them so we wouldn't artificially inflate the total network volume. We preserved the exact 20,000 unique transactions.
...
We built a LangGraph engine to hit the 30-point AI bonus rubric:
...
*   Dynamic Charting (10 pts): It writes secure DuckDB SQL in real-time, executes it, and uses Matplotlib to draw the correct chart type (Bar, Line, Pie, Scatter).
