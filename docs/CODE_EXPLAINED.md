# Deep Dive: Our Code & Methodology
**TransOrg AgentIQ Datathon**

This document explains exactly how our data pipeline, graph engine, and AI copilot work under the hood. We built this to be completely transparent for the judges.

---

## 1. Data Cleaning (Pandas)
We used the `pandas` library heavily in `backend/pipeline.py` and our Jupyter notebooks to fix the messy data without just recklessly dropping rows.
...
*   **Fixing Currency:** The `amount` column had weird symbols and commas. We stripped everything except numbers and decimals, and cast it to a `float`.
...
*   **Deduplication:** We found exactly 400 perfectly identical duplicate rows in the raw transaction file. We dropped them so we wouldn't artificially inflate the total network volume. We preserved the exact 20,000 unique transactions.

## 2. Fraud Ring Detection (NetworkX)
We didn't just run SQL queries. We used `NetworkX` in `backend/fraud_ring_detection.py` to build graphs and find hidden relationships:

*   **Merchant Collusion:** We grouped merchants by their `settlement_account`. If 5 different merchants are sending money to the exact same bank account, it's a collusion ring. We found 203 of these.
...
### Question: "Why did you build an AI Copilot?"
